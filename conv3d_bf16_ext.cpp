...
std::unordered_map<int64_t, std::shared_ptr<Conv3dBf16Context>> g_contexts;
...
std::shared_ptr<Conv3dBf16Context> get_context_or_throw(int64_t handle_id) {
    std::lock_guard<std::mutex> lock(g_ctx_mutex);
    auto it = g_contexts.find(handle_id);
    if (it == g_contexts.end()) {
        throw std::invalid_argument("Invalid conv3d_bf16 handle_id.");
    }
    return it->second;
}

std::pair<int64_t, int64_t> autotune_best_plan(const std::shared_ptr<Conv3dBf16Context>& ctx) {
    constexpr int kWarmupIterations = 3;
    constexpr int kProfileIterations = 20;

    auto bf16_options = torch::TensorOptions().dtype(torch::kBFloat16).device(torch::kCUDA, ctx->device_index);
    auto ws_options = torch::TensorOptions().dtype(torch::kUInt8).device(torch::kCUDA, ctx->device_index);

    auto x = torch::randn({ctx->n, ctx->c, ctx->d, ctx->h, ctx->w}, bf16_options)
                 .to(torch::MemoryFormat::ChannelsLast3d);
    auto w = torch::randn({ctx->k, ctx->c, ctx->t, ctx->r, ctx->s}, bf16_options)
                 .to(torch::MemoryFormat::ChannelsLast3d);
    auto y = torch::empty({ctx->n, ctx->k, ctx->out_d, ctx->out_h, ctx->out_w},
                          bf16_options.memory_format(torch::MemoryFormat::ChannelsLast3d));
    auto bias = torch::zeros({1, ctx->k, 1, 1, 1}, bf16_options);
    ...
    if (best_plan_index < 0) {
        throw std::runtime_error("conv3d_bf16 autotune failed to find a runnable execution plan.");
    }

    return {best_plan_index, best_workspace_bytes};
}

int64_t conv3d_bf16_init(
    ...
    auto ctx = std::make_shared<Conv3dBf16Context>();
    ctx->n = x_shape[0];
    ctx->c = x_shape[1];
    ctx->d = x_shape[2];
    ctx->h = x_shape[3];
    ...
    auto conv_options = fe::graph::Conv_fprop_attributes()
                            ...
                            .set_name("conv3d_bf16")
                            .set_compute_data_type(fe::DataType_t::FLOAT);

    auto conv_output = ctx->graph->conv_fprop(ctx->x_attr, ctx->w_attr, conv_options);
    ...
    if (ctx->graph->get_execution_plan_count() <= 0) {
        throw std::runtime_error("No execution plan is available for conv3d_bf16.");
    }
...
torch::Tensor conv3d_bf16_forward(
    int64_t handle_id,
    const torch::Tensor& x,
    const torch::Tensor& w,
    const c10::optional<torch::Tensor>& bias) {
    auto ctx = get_context_or_throw(handle_id);
    if (!x.is_cuda() || !w.is_cuda()) {
        throw std::invalid_argument("All tensors passed to conv3d_bf16.forward must be CUDA tensors.");
    }
    if (x.get_device() != ctx->device_index || w.get_device() != ctx->device_index) {
        throw std::invalid_argument("All tensors passed to conv3d_bf16.forward must be on the init device.");
    }
    if (x.scalar_type() != torch::kBFloat16 || w.scalar_type() != torch::kBFloat16) {
        throw std::invalid_argument("x and w dtype must be torch.bfloat16.");
    }
    if (x.dim() != 5 || w.dim() != 5) {
...
void conv3d_bf16_destroy(int64_t handle_id) {
    std::lock_guard<std::mutex> lock(g_ctx_mutex);
    auto it = g_contexts.find(handle_id);
    if (it != g_contexts.end()) {
        g_contexts.erase(it);
...
TORCH_LIBRARY(conv3d_bf16, m) {
    m.def("init(int[] x_shape, int[] w_shape, int device_index, int[] padding, int[] stride, int[] dilation, bool with_bias=False) -> int");
    m.def("forward(int handle_id, Tensor x, Tensor w, Tensor? bias=None) -> Tensor");
    m.def("destroy(int handle_id) -> ()");
}

TORCH_LIBRARY_IMPL(conv3d_bf16, CUDA, m) {
    m.impl("init", &conv3d_bf16_init);
    m.impl("forward", &conv3d_bf16_forward);
    m.impl("destroy", &conv3d_bf16_destroy);
}

PYBIND11_MODULE(conv3d_bf16_ext, m) {
    m.doc() = "Prebuilt conv3d_bf16 Torch extension module.";
    m.def(
        "init",
        &conv3d_bf16_init,
