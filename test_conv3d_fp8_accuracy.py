...
def run_conv3d_fp8_accuracy_check():
    ...
    with torch.no_grad():
        y_fp8 = op.forward(input_fp8, weight_fp8, descale_x, descale_w, bias=None)
        y_ref = F.conv3d(
...
def run_conv3d_fp8_accuracy_check_with_bias():
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available.")
    if not hasattr(torch, "float8_e4m3fn"):
        raise RuntimeError("Current PyTorch does not expose float8_e4m3fn.")
    ...
    with torch.no_grad():
        y_fp8 = op.forward(input_fp8, weight_fp8, descale_x, descale_w, bias=bias_bf16)
        y_ref = F.conv3d(
            input_fp8.to(torch.bfloat16),
            weight_fp8.to(torch.bfloat16),
            bias=bias_bf16,
    ...
    print(
        "Accuracy check against torch.conv3d (with bias): "
        f"max_abs={max_abs:.6f}, mean_abs={mean_abs:.6f}, max_rel={max_rel:.6f}, "
        f"y_ref_max_abs={y_ref_max_abs:.6f}, y_ref_mean_abs={y_ref_mean_abs:.6f}"
    )
...
def test_conv3d_fp8_accuracy_against_torch_conv3d_with_bias():
    max_abs, mean_abs, _, _, _ = run_conv3d_fp8_accuracy_check_with_bias()
...
def main():
    ...
    max_abs_bias, mean_abs_bias, _, y_ref_max_abs_bias, y_ref_mean_abs_bias = run_conv3d_fp8_accuracy_check_with_bias()
    # if max_abs_bias >= 1.0 or mean_abs_bias >= 0.1:
    #     raise RuntimeError(
    #         "Accuracy check with bias failed: "
    #         f"max_abs={max_abs_bias:.6f}, mean_abs={mean_abs_bias:.6f}, "
    #         f"y_ref_max_abs={y_ref_max_abs_bias:.6f}, y_ref_mean_abs={y_ref_mean_abs_bias:.6f}"
    #     )
    print("conv3d_fp8 accuracy check with bias passed.")
